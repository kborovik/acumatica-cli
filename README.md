...
`acu config init` scaffolds a **single full seed** under `config/` (Bootstrap `project.xml` at `Bootstrap/1.1.0`, expanded COA, masters) plus lifecycle `scenario/`, observer `config/views/`, and README.
There is no `--flavor`.
...
| `Bootstrap/1.1.0` or `Default/25.200.001` | literal pin (ignores the resolved Default version) |
