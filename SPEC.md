...
T31|.|graduate T29 scratch into data-repo `baseline/` — author `10-subaccounts.yaml` + `20-accounts.yaml` (real chart-of-accounts values, not scratch 8710/8720; `AccountGroup` + `RevaluationRateType` stripped — fresh tenant lacks PM account groups) + `30-currencies.yaml` (`endpoint: Bootstrap/1.0.0` override; Translation* pairs omitted while translation feature off — write-tolerated, read-invisible); numbered prefixes encode apply order (subaccounts < accounts < currencies < currency-denominated accounts; dir expansion sorts alphabetical); verify: provision fresh scratch tenant zero manual steps → clean diff over everything applied, currencies included|V2,V4,V22,I.data,T29,T30
