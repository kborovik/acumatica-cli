...
- data: `baseline/*.yaml` → `entity` / `key` (string or list) / `records` + `endpoint` ? (per-file contract-endpoint override, e.g. `Bootstrap/1.1.0`); parsed by `seed.py`
...
T34|.|extend Bootstrap endpoint w/ GLPreferences entity (GL102000) + bump 1.1.0 → 1.2.0 (V21 ripple: `bootstrap_project.xml` + `seed.py` docstring + `templates/bootstrap/*.yaml` + test fixtures + data-repo `endpoint:` refs + §I.data example); author data-repo `40-ledger.yaml` (Default-endpoint `Ledger` entity, ACTUAL, sorts after `20-accounts`) + `50-gl-preferences.yaml` (`endpoint: Bootstrap/1.2.0`; YTD net income + retained earnings accounts from `20-accounts`; strip server-derived fields per V22); verify: provision fresh scratch tenant zero manual steps → Finance screens open (no `PXSetupNotEnteredException`) + GL batch posts + diff clean|V20,V21,V22,T31,T33,I.data
T35|.|graduate minimal generic company into `config init` templates — copy T34-verified minimal set into package templates (`10-subaccounts.yaml` single ZERO segment + `20-accounts.yaml` ~10 generic accounts incl retained earnings + YTD net income + `40-ledger.yaml` + `50-gl-preferences.yaml`; existing uoms example → `90-uoms.yaml`), numbered prefixes teach V22 ordering; verify: empty dir → `init --host h` → provision fresh tenant zero manual steps → GL batch posts + diff clean; re-run init → all `skip`, exit 0|V2,V22,T26,T34
