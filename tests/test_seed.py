...
def test_load_baseline_rejects_bootstrap_entity_without_endpoint(
    ...
    with pytest.raises(
        SystemExit,
        match=r"endpoint: default.*Bootstrap/1\.6\.0.*'bootstrap' \| 'default'",
    ):
...
def test_load_baseline_bootstrap_entity_explicit_endpoint_passes(
    ...
    for endpoint in ("Bootstrap/1.6.0", "Default/25.200.001", "default"):
...
def test_apply_company_relogins_once_per_session(
    ...
    assert methods_paths == [
        ...
        ("PUT", "/AcumaticaERP/entity/Bootstrap/1.6.0/Company"),
