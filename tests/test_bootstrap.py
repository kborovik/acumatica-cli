...
def test_package_zip_carries_the_bootstrap_endpoint() -> None:
    ...
    views: dict[str, dict[str, str | tuple[str, str]]] = {
        "Company": {
            ...
            "DecPlQty": "commonsetup",
            "WeightUOM": "commonsetup",
            "VolumeUOM": "commonsetup",
        },
        ...
        # T204/T207 SegmentedKey (gh #30 / B28): CS202000 DimensionMaint.
        # Header (Dimension) + Detail (Segment). Flat entity; one segment
        # row per DimensionID. Live aspx DataMember is Detail (singular).
        "SegmentedKey": {
            "DimensionID": "Header",
            "SegmentID": "Detail",
            "Length": "Detail",
...
def test_role_users_maps_to_users_by_role() -> None:
    ...
    assert mappings["Users"] is not None
    assert mappings["Users"].get("object") == "UsersByRole"
    assert mappings["Users"].get("field") == ""
    details = {d.get("name"): d for d in endpoint.findall(f"{ns}Detail")}
    role_user = details["RoleUser"]
    fields = {f.get("name") for f in role_user.findall(f"{ns}Fields/{ns}Field")}
    assert fields == {"Username"}
    user_maps = {
        m.get("field"): m.find(f"{ns}To")
        for m in role_user.findall(f"{ns}Mappings/{ns}Mapping")
    }
    assert user_maps["Username"] is not None
    assert user_maps["Username"].get("object") == "UsersByRole"
...
def test_segmented_key_maps_length_to_detail_view() -> None:
    ...
    assert mappings["DimensionID"] is not None
    assert mappings["DimensionID"].get("object") == "Header"
    assert mappings["SegmentID"] is not None
    assert mappings["SegmentID"].get("object") == "Detail"
    assert mappings["Length"] is not None
    assert mappings["Length"].get("object") == "Detail"
    assert mappings["Length"].get("field") == "Length"
