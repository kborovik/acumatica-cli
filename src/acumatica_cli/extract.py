...
Hard-cut emit under acu-config/{bootstrap,baseline,setup,master}/ (V30/V34):
...
acu-config/bootstrap/features.yaml is the feature closure (V22/B15): the
...
FEATURES_FILE = "acu-config/bootstrap/features.yaml"
...
_TEMPLATE_SEED_EXEMPT_PREFIXES = ("acu-config/views/",)


def packaged_template_seed_files() -> frozenset[str]:
    """Packaged ``templates/acu-config/**`` seed paths V34 requires catalog rows for.

    Paths are data-repo relative (``acu-config/...``). Excludes features synthesis,
    ``project.xml``, and ``acu-config/views/`` observer defs.
    """
...
def _synth_open_periods(client: AcumaticaClient) -> dict[str, Any] | None:
    ...
    # resolves inside the emitted set (V22 - acu-config/bootstrap/company.yaml
...
def render_features(gates: Iterable[str]) -> str:
    """The feature-closure acu-config/bootstrap/features.yaml: built-in six + gates.
...
def run(
    ...
    Paths hard-cut under acu-config/ SEED_DIRS (V30). Per file: skip when it
